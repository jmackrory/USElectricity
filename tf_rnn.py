...
class simpleRecurrentNeuralNetwork(NNModel):
    ...
    Super simple thing, currently preserved in EBA_RNN_Old.
    ...
    def get_random_batch(X,y,n_batch,seq_len):
        ...
        y_list=[]
        for i in range(self.config.Nbatch):
            n0=int(np.random.random()*(Nt-self.config.Ntime_out-1))
            n1 = n0+self.config.Ntime_out
            x_sub = X[n0:n1]
            y_sub = y[n0:n1]
            x_list.append(x_sub)
            y_list.append(y_sub)
        x_batch=np.stack(x_list,axis=0)
        y_batch=np.stack(y_list,axis=0)
        y_batch=y_batch.reshape( [self.config.Nbatch,self.config.Ntime_out,-1])                    
        return x_batch,y_batch

    
    def model_predict_whole(self,Xin,path_str="pdx_RNN_model"):
