...
def make_airport_df():
     ...
     airport_df = airport_df[msk2][['name','municipality','ident']].rename(columns={'ident':'CALL', 'municipality': 'City'})
...
def get_airport_code(dataframe, city_list, depth=3):
    ...
    #         city = city_list[i][j]
    #         state = city_list[i][0]
    #         msk = dataframe['municipality'].str.contains(city)
    ...
    #             elif (state == 'ME'):
    #                 call = 'KPWM'
    #             msk=dataframe['ident'].str.contains(call)
    ...
    #         aircode_df = aircode_df.append(df)
    #aircode_df = dataframe
    msk2 = ((dataframe['iso_country'] == 'US') &
            dataframe['type'].apply(lambda x: x in ['large_airport', 'medium_airport']))
    aircode_df = dataframe[msk2]
    aircode_df = aircode_df.rename(columns={'ident':'CALL', 'municipality': 'City', 'name':'Name'})
    return aircode_df
...
def get_all_data(aircode, start_year=2015, end_year=2020):
    ...
    for year in range(start_year, end_year):
        for i in tqdm(range(len(aircode))):
            ...
            city = ap['City']
            airport = ap['name']
            wget_data(usaf, wban, str(year), city, airport)
    return None
