...
class NNModel(object):
    ...
    def add_placeholders(self):
        ...
        self.X = tf.placeholder(
            tf.float32, [None, self.config.Nsteps_in, self.config.Ninputs], name="X"
        )
        # Outputs: n_outputs we want to predict in the future.
        self.y = tf.placeholder(
            tf.float32, [None, self.config.Nsteps_out, self.config.Noutputs], name="y"
        )
    ...
    def get_random_batch(self, X, y):
        ...
        for i in range(self.config.Nbatch):
            ...
            n0 = int(
                np.random.random()
                * (Nt - self.config.Nsteps_in - self.config.Nsteps_out)
            )
    ...
    def add_prediction_op(self):
        ...
        A = tf.Variable(
            (
                1
                + tf.random_uniform(
                    (self.config.Ninputs, self.config.Nhidden), -0.1, 0.1
                )
                / self.config.Ninputs
            ),
            name="A",
        ...
        rnn_outputs, states = tf.nn.dynamic_rnn(
            multi_cell, inputs_reduced, dtype=tf.float32
        )
        ...
        A_out = tf.Variable(
            (1 + tf.random_uniform((Nt2, Nt1), -0.1, 0.1)) / Nt2,
            name="A_out",
            trainable=True,
        )
        remapped_outputs = tf.matmul(stacked_rnn_outputs, A_out)
        outputs = tf.reshape(
            remapped_outputs, [-1, self.config.Nsteps_out, self.config.Noutputs]
        )
...
class simpleRecurrentNeuralNetwork(NNModel):
    ...
    def add_prediction_op(self):
        ...
        stacked_outputs = fully_connected(
            stacked_rnn_outputs, self.config.Noutputs, activation_fn=None
        )
        outputs = tf.reshape(
            stacked_outputs, [-1, self.config.Nsteps_out, self.config.Noutputs]
        )
        return outputs
