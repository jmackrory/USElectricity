...
class multiseasonal_temp(object):
    ...
    def optimize_param(self, y, T, rtol=0.01, eta=0.001, lr=0.05, nmax=1000):
        """optimize_param
    ...
    def calc_param_grad(self, y, T, pred):
        ...
        dparam = {}
        dparam["alpha"] = (
            np.sum(eps[self.Ninit + self.N0 :] * eps_level[self.Ninit : -self.N0]) / Nt
        )

        dparam["beta"] = (
            np.sum(eps[self.Ninit + self.N0 :] * eps_grad[self.Ninit : -self.N0]) / Nt
        )
        ...
        dparam["g00"] = (
            np.sum(
                msk0[self.Ninit + self.N0 :]
                * msk0[self.Ninit : -self.N0]
                * eps2[self.Ninit : -self.N0]
            )
            / Nt
        )
        dparam["g01"] = (
    ...
    def plot_param_update(self, y, T, skip=960, Nupdate=3):
        ...
        plt.legend(["Season", "Demand-T", "Trend"], loc="lower right", prop={"size": 9})
