...
def avg_extremes(df, window=2):
    ...
    msk = msk1 | msk2
    print(
        "Number of extreme values {}. Number of zero values {}".format(
            sum(msk1), sum(msk2)
        )
    )
...
def make_seasonal_plots(dem, temp, per, nlags):
    ...
    dem_mu = dem.mean()
    dem_trend = (
        dem_decomposition.trend / dem_mu
    )  # Find rolling average over most important period.
    dem_seasonal = (
        dem_decomposition.seasonal / dem_mu
    ...
    temp_mu = temp.mean()
    temp_trend = (
        temp_decomposition.trend / temp_mu
    )  # Find rolling average over most important period.
    temp_seasonal = (
        temp_decomposition.seasonal / temp_mu
    )  # Find the dominant frequency components
...
def fft_detrend(dem_f, f, width, remove_func):
