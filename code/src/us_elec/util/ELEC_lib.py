...
def plot_generation_by_state(cur, state):
    ...
    gen_labels = (
        state_gen.iloc[plt_ind]["name"]
        .str.extractall("Net generation : ([\s\w\(\)-]+):")[0]
        .values
    )

    plot_data_frame(
        state_gen.iloc[plt_ind],
        xlabel="Date",
        ylabel="Net Generation (GWh)",
        title="Generation Across " + state + " by source",
        labels=gen_labels,
        logy=False,
    )
...
def get_state_data(cur, year):
    ...
    for i, state in enumerate(state_abbr):
        for j, gen in enumerate(gen_types):
            ...
            WHERE series_id LIKE 'ELEC.GEN.{ser}-{state}-99.A' AND obs_date= DATE '{year}-12-31'""".format(
                ser=gen, state=state, year=year
            )
            cur.execute(sql_str)
            r = cur.fetchone()
            if r is not None:
